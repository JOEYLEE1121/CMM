...
def submit_order(order: Order):
    ...
    try:
        submitted_order = api.submit_order(order.symbol, order.quantity, order.side, "limit", "gtc", limit_price)
    ...
    WEBHOOKS[order.strategy].send(
        ...
        file=file_from_text(json.dumps(submitted_order._raw, indent=4), filename="order.json"),
    )